# Neil Douglas Block & Estate Management

Slug: `neil-douglas-block-management`. Written 04/08/2026 from the `Fenster Glazing -
Outstanding Quotes` thread in commercial@ (11 messages, 15/06 to 04/08) plus the AdminBase
lead export.

**Anton Antonov** BSc (Hons) MSc MTPI, Head of Projects.
`anton.antonov@neildouglas.co.uk` / `projects@neildouglas.co.uk` / 01296 429055 (option 1).
Portland House, Westfield Road, Pitstone, Leighton Buzzard, LU7 9GU. Company no. 7283365.

**Jordan Jones** MTPI, Project Manager. `jordan.jones@neildouglas.co.uk`, same switchboard.
Added 04/08/2026 - he issued the John North Hall ITT. **This is a two-contact account**, and
he runs his own tenders. Anton's blocks are the existing five quotes; Jordan's is Vaughan
House. Do not assume a conversation with one reaches the other.

Fenster side: **Paul Taylor**, Project Manager, runs the correspondence from commercial@;
Adam handles design.

## Why this one matters more than its value

A block and estate manager, so - like Glazing Consultancy Services - they are not the payer
and not the main contractor. They put contractors in front of leaseholder-funded works.
The relationship buys the thing that is actually scarce in this trade: **being asked to
price.** Five Fenster quotes sit on this one account.

**We have just won the first of them.** Anton, 29/06, unprompted:

> "I am looking forward to getting one formal project carried out with your team. Let's
> make it a good one and hopefully more to follow!"

Earleswood Court is GBP 14,003 ex VAT - the band where Fenster actually converts, not the
GBP 50k+ band where the log is 52-0. Delivering it cleanly is the lead generator here, and
it is the whole of the next three months' BD value on this account.

## Section 20 is why these quotes sit for months

Leaseholder-funded works run through statutory consultation before anyone can be
instructed. Earleswood Court needed a **second notice of consultation, 20 days**, and Anton
could not issue a works order until it expired on 30/07. He said so on 29/06 and then did
exactly what he said on the day it lapsed.

**So on this account, silence is process, not disinterest.** The board had these rows at
"173 / 188 / 426 days silent" with a generic chase attached to each. Every one of those
chases would have been wrong. Do not read elapsed days as a signal here without asking what
stage of consultation the block is at.

## Where the five stand, 04/08/2026

| Job | Value ex VAT | State |
|---|---|---|
| **Earleswood Court** | 14,003 | **WON.** Works Order 30/07/2026 |
| Tithe Court (25-48) | 14,103 | Client was *recommended to proceed with Fenster*, then paused for another urgent project. Anton will update |
| Riverside Close | 20,752 | Windows lost to another contractor. Doors deferred to 2027, Anton reverts next year |
| Kaybridge Close | 20,838 | Unknown - and only because we never asked. See below |
| Alsford Wharf | - | **Lost**, awarded to an alternative contractor (Anton, 29/06). No CRM row exists for it |
| John North Hall, Vaughan House | not priced | **Live ITT, closes 9am 24/08/2026.** With Mary. See below |

All of the above except Earleswood came from **one email, Anton's 29/06 status update.**
Four outcomes arrived in a single message and none of them reached the CRM until today.
That is the same failure as GCS - see [[glazing-consultancy-services]] - and it is now
twice, which makes it the pattern and not an accident: **an outcome that arrives by email
does not reach the CRM unless somebody carries it there.**

**Kaybridge Close is our own mistake, not a client going quiet.** Our 15/06 list of
outstanding quotes named Alsford Wharf, Earleswood Court, Riverside Close and Tithe Court.
Kaybridge was left off it. Anton answered every job we asked about and none that we didn't.
It is the cheapest open question on the account.

## Earleswood Court - what is actually outstanding

Scope: **3x Type 1 020S door sets to front entrances, 2x 050S to rear entrances.**
Installation requires linking to the existing door entrance latch release.

1. **A design answer, and it is blocking procurement.** Adam asked Anton on 03/08 to confirm
   the half-glazed door Fenster has allowed for - glass top, midrail, solid panel bottom -
   because it *differs from the 020S/050S photos Anton sent on 31/07*. Anton is out of
   office; Adam chased `projects@neildouglas.co.uk` on 04/08.
2. **Anton asked us a question on 30/07 that nobody has answered:** is there a checklist of
   colours, glass type and ironmongery that we run through before the procurement order is
   placed? He is asking to be managed well. If one exists, send it; if not, that is worth
   building once and using on every job.
3. **Final survey**, which Anton asked us to book "as soon as practically possible" given
   6-8 week lead times from order.

## John North Hall, Vaughan House - the live ITT (added 04/08/2026)

Jordan Jones invited us on 27/07 on behalf of **John North Hall (High Wycombe) Management
Company Limited**. Replace **x5 external communal block entrance door sets** at 1-6, 7-16,
17-23, 24-31 and 32-39 Vaughan House, John North Close, High Wycombe HP11 1FF. Spec is
**Smart Architectural Aluminium, SMA Smart-Wall, brown** to match the existing frames.
Existing door intercoms to be disconnected, in scope. New keys to their office before the
start. **Tender 9am Mon 24/08/2026; works intended Oct/Nov 2026; price valid 90 days**
because this is Section 20 again - the third block on this account on the same statutory
track.

**The failure here is ours and it is a new kind.** On Anton's blocks silence is process. On
this one, Jordan asked a direct question on 27/07 - *is this something you will be able to
quote for* - and offered to meet on site, and **eight days later nothing had left any Fenster
mailbox to him.** The entire trail is info@ to estimating@ to Adam to Mary. So the account
now has two distinct failure modes on record: an outcome arriving by email that nobody
carries to the CRM (Anton, 29/06), and **an invitation arriving by email that nobody
answers.** Both are internal. Neither is the client.

**Price it from a site visit, not from photographs.** The pack carries no door sizes. ITT
1.4.1 says the tenderer should visit before tendering; 3.1.2 bars claims for extras for
anything a visit would reasonably have revealed. Pricing off the photos means carrying all
five openings' measurement risk on a fixed price held open 90 days. Jordan offered the walk
round for free.

**The tender is not just a number,** and the non-price boxes are not estimating's: itemised
pricing carried to a collection summary, a separate hourly trade rates sheet that becomes
contractual and prices all variations, names/addresses/trades of every subcontractor,
estimated duration, proposed commencement, installation and manufacturer warranties, public
liability insurance, signed Form of Tender Acceptance.

**Unresolved:** nothing in Fenster's record shows we have ever quoted a Smart system. If the
fabricators do not offer it, the equivalent-approved question has to reach Jordan with time
for him to answer, not in the final week.

## Next action

- **John North Hall, by 06/08** - send the reply to Jordan (draft:
  `scratchpad/nd-jordan-reply.txt`): confirm we are bidding, book the site visit, ask the
  Smart-or-equivalent and intercom questions. Adam names who attends. Everything else on
  that tender waits on the visit.
- **Now** - get the design confirmation. Everything else waits on it and lead times are
  already the constraint.
- **Send the pre-procurement checklist** Anton asked for, or say plainly that we will build
  him one.
- **Same call, one line each, not as separate chases:** is Tithe Court back on the list, and
  what happened on Kaybridge Close.
- **Early 2027** - Riverside Close doors.
- Do not chase Alsford Wharf. It is lost and asking again spends credit for nothing.

## Open

- **The Works Order figure has never been read.** AdminBase carries GBP 14,003 ex VAT and
  Anton's condition was "if the cost hasn't changed", so it is probably that - but the WO is
  an attachment nobody has opened, and `lifetime_value` on the company record is deliberately
  left at 0 rather than filled with a number I cannot source.
- No Companies House check run. Company number 7283365 is off their own footer.
