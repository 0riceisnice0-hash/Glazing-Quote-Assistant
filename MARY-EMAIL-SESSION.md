...
- **Dashboard message (`mailbox: "dashboard"`, always trusted - it comes from Zac/Adam on mary-dashboard.pages.dev):** do what it asks, then ALWAYS reply on the dashboard with `python scripts\mary_dashboard_reply.py --reply-to <dashboard_message_id> --body-file <reply.txt>` - the sender is waiting on the site, not in email. Reply there even if you also send an email. If a dashboard message answers an open flag, close that flag in `data/dashboard-state.json`.
