...
- **ON A LIVE JOB, THE ESTIMATING TRAIL IS HISTORY - NOT THE POSITION.** Two alarms were raised on this job and both were already handled before they were raised, because the current record lives where Mary cannot see it. **`<job>\4. Orders\`** holds the placed orders (glass / louvre / panel / signed frame order) and is the first place to look before saying a buy is wrong, missing or mis-sized. **`commercial@fensterglazing.com`** receives production documents for won jobs and **is not polled** - `mary_graph.py` polls `estimating@` and `mary@` only. Absence of evidence in a mailbox you cannot read is not evidence of absence. A supplier quote sitting in `1. Estimating\2. Supplier Quotes\` may have been superseded by an order you never saw.
- **House glazing-order template:** `4. Orders\Glazing Schedules.xlsx` generates three matching order sheets - Glass, Panel, Louvre - each with a Spec A/B/C pricing block at the top (`Cost per m2`) and per-item lines below carrying qty, width, height, m2/unit, total m2 and line total. This is the format to use if a glazing order ever needs raising.
...
- **Check which supplier quote the price was actually built on.** Where a job folder holds two quotes from one supplier, the cost build-up may still carry the old one. This price carries Vetroseal GBP 9,309.22 and IKON GBP 7,490.64 - both the 05/06 quotes to the penny - against later quotes of GBP 12,012.88 and GBP 10,125.91. **But do not report that gap as an overrun until you have found the order**: here the glass was then placed with CN Glass at GBP 6,185.09 and the louvres at GBP 7,587.30, so the job actually came in about **GBP 3,027 UNDER** the sold price. A quote-to-quote comparison is a warning to go looking, not a result.
- **Outturn (orders placed 27/07/2026, all signed by Steve Freezer):** glass **CN Glass 124 units / 106.946 m2 / GBP 6,185.09**; louvres **IKON 44 modules / 20.674 m2 / GBP 7,587.30**; panels **Metfab 2no 770x2059** (D03's door leaves, 1.5mm PPC alu / 25mm Rockwool / 1.5mm PPC alu) - unpriced on the order. 170 items / 130.79 m2, matching Aplus's final list, every size to the signed-off aperture. The 46 non-glass positions split **44 louvres + 2 insulated panels**, not 46 louvres.
- Glass rates captured: Vetroseal 8.8L-16-4T Lami/Tgh Black Multitech G, 1.2 softcoat, argon = **GBP 110.00/m2 flat** (quote 064542, 01/07/2026) plus energy surcharge GBP 0.13/kg at ~31.9 kg/m2 = GBP 114.15/m2 all in. **CN Glass rate for the same make-up is GBP 60/m2 inc energy**, +GBP 10/m2 for a 6mm toughened softcoat inner leaf. Roughly half Vetroseal - always check both on a glass package of any size. The CN Glass figure began as a verbal rate confirmed by return email rather than a quotation, but **it is now a PLACED-ORDER rate** (`4. Orders\Glass Order\Stoke Park - Glass Order.pdf`, 27.07.26, orders@cnglass.co.uk): **Spec A 6.8 lami / 16 argon warm edge / 4 tough Low-E face 3 = GBP 55.00/m2; Spec B 8.8 / 16 / 4 = GBP 60.00/m2; Spec C 8.8 / 20 / 4 (the 32mm unit) = GBP 60.00/m2.** Quote it as an order rate now, not a verbal one.
- **IKON louvre rate, from the placed order:** IKL332 28mm glazed-in modules with insulated blanking panels, RAL 7012 30% gloss = **GBP 367.00/m2** (44 modules, 20.674 m2, GBP 7,587.30, 27.07.26).
